...
class UrlQuerySanitizerTest {
  ...
  // the main project\'s HttpConstants.SENSITIVE_QUERY_PARAMETERS
  ...
  private static final Set<String> TEST_SENSITIVE_PARAMS =
      new HashSet<String>(asList("secret", "apiKey", "token"));
  ...
  void redactQueryString_shouldReturnOriginalWhenNoSensitiveParams() {
    assertThat(UrlQuerySanitizer.redactQueryString("secret=value&apiKey=key", java.util.Collections.<String>emptySet()))
  ...
  void redactUrl_shouldReturnOriginalWhenNoSensitiveParams() {
    ...
    assertThat(UrlQuerySanitizer.redactUrl(url, java.util.Collections.<String>emptySet())).isEqualTo(url);
  }
