...
import static io.opentelemetry.java6.agent.instrumentation.jdbc.sql.SqlDialect.DOUBLE_QUOTES_ARE_IDENTIFIERS;
import static io.opentelemetry.java6.agent.instrumentation.jdbc.sql.SqlDialect.DOUBLE_QUOTES_ARE_STRING_LITERALS;
...
/** Ported from the main project (v2.31.1); runs on Java 6 via MiniJupiter. */
class SqlQueryAnalyzerTest {
  ...
  void sanitizeSql(String original, String expected, String expectedQuerySummary) {
    ...
    if (emitStableDatabaseSemconv()) {
      assertThat(querySummary(result)).isEqualTo(expectedQuerySummary);
    } else {
      assertThat(querySummary(result)).isNull();
    }
  }
  ...
  void simplifySql(String original, Function<String, SqlQuery> expectedFunction) {
    ...
    if (emitStableDatabaseSemconv()) {
      ...
      assertThat(querySummary(result)).isEqualTo(querySummary(expected));
    } else {
      assertThat(result.getOperationName()).isEqualTo(expected.getOperationName());
      assertThat(result.getCollectionName()).isEqualTo(expected.getCollectionName());
      assertThat(querySummary(result)).isNull();
  ...
  void sanitizeGrantObjectNamedPasswordWithSummary() {
    ...
    assertThat(result.getQueryText()).isEqualTo("GRANT SELECT ON password TO admin");
    assertThat(querySummary(result)).isEqualTo("GRANT");
  }
  ...
  void sanitizeDdlObjectNamedPasswordWithSummary() {
    ...
    assertThat(result.getQueryText()).isEqualTo("CREATE USER password PASSWORD ?");
    assertThat(querySummary(result)).isEqualTo("CREATE USER password");
  }

  private static List<Arguments> sensitiveArgs() {
  ...
  void veryLongSelectStatementsAreOk() {
    ...
    if (emitStableDatabaseSemconv()) {
      ...
      assertThat(querySummary(result)).isEqualTo("SELECT table");
    } else {
      assertThat(result.getOperationName()).isEqualTo("SELECT");
      assertThat(result.getCollectionName()).isEqualTo("table");
      assertThat(querySummary(result)).isNull();
  ...
  void veryLongNumbersAreOk() {
    ...
    SqlQuery result = ANALYZER.analyze(s, DOUBLE_QUOTES_ARE_STRING_LITERALS);
  ...
  void test32kTruncation() {
    ...
    if (emitStableDatabaseSemconv()) {
      assertThat(querySummary(result)).startsWith("SELECT TABLE SELECT TABLE SELECT TABLE");
    } else {
      assertThat(querySummary(result)).isNull();
    }
  }
  ...
  void randomBytesDontCauseExceptionsOrTimeouts() {
    ...
    for (int i = 0; i < 1000; i++) {
      ...
      ANALYZER.analyze(sb.toString(), DOUBLE_QUOTES_ARE_STRING_LITERALS);
    }
  }
  ...
  void querySummaryIsTruncated() {
    ...
    String result =
        querySummary(
            ANALYZER.analyzeWithSummary(sql.toString(), DOUBLE_QUOTES_ARE_STRING_LITERALS));
  ...
  private static List<Arguments> operationCaseArgs() {
    return java.util.Arrays.asList(
  ...
  private static List<Arguments> sqlArgs() {
    return java.util.Arrays.asList(
        Arguments.of(
  ...
  private static Function<String, SqlQuery> expect(
      final String operation, final String collectionName, final String querySummary) {
  ...
  private static Function<String, SqlQuery> expect(
      ...
      final String stableOperationName,
      final String stableCollectionName,
      final String querySummary) {
    return new Function<String, SqlQuery>() {
      public SqlQuery apply(String sql) {
        return SqlQuery.create(sql, operation, collectionName);
      }
    };
  }

  private static Function<String, SqlQuery> expect(
      final String sql, final String operation, final String collectionName, final String querySummary) {
  ...
  private static Function<String, SqlQuery> expect(
      ...
      final String querySummary) {
    return new Function<String, SqlQuery>() {
      public SqlQuery apply(String ignored) {
        return SqlQuery.create(sql, operation, collectionName);
      }
    };
  }
  ...
  private static Function<String, SqlQuery> expectStoredProcedure(
      final String operation, final String storedProcedureName, final String querySummary) {
    return expectStoredProcedureWithStableOperation(
        operation, storedProcedureName, operation, querySummary);
  }

  private static Function<String, SqlQuery> expectStoredProcedure(
      final String sql, final String operation, final String storedProcedureName, final String querySummary) {
    return new Function<String, SqlQuery>() {
      public SqlQuery apply(String ignored) {
        return SqlQuery.create(sql, operation, storedProcedureName);
      }
    };
  }
  ...
  private static List<Arguments> simplifyArgs() {
    return java.util.Arrays.asList(
        // Select
        Arguments.of(
  ...
  private static List<Arguments> ddlArgs() {
    return java.util.Arrays.asList(
        Arguments.of(
            "CREATE TABLE `table`",
  ...
  private static List<Arguments> doubleQuotesAsIdentifiersArgs() {
    return java.util.Arrays.asList(
  ...
  // --- Java 6 port support: the agent implements the old (default) database semconv only ---

  private static boolean emitStableDatabaseSemconv() {
    return false;
  }

  private static String querySummary(SqlQuery query) {
    return null;
  }

  private static SqlQuery createWithSummary(Object... ignored) {
    throw new UnsupportedOperationException("stable semconv");
  }
  ...
  static final class Analyzer {
    SqlQuery analyze(String sql, SqlDialect dialect) {
      return SqlQueryAnalyzer.analyze(sql, dialect);
    }

    SqlQuery analyzeWithSummary(String sql, SqlDialect dialect) {
      return SqlQueryAnalyzer.analyze(sql, dialect);
    }
  }
}
